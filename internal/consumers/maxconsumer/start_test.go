...
func TestMaxConsumer_Start(t *testing.T) {
	t.Parallel()
	...
	for _, testCase := range tests {
		...
		t.Run(tc.description, func(t *testing.T) {
			...
			config := buildMaxConsumerConfig(tc.expectedRecordsConsumed)

			consumer := maxconsumer.NewMaxTempConsumer(dataChannel, mockLogger, config)
...
func buildMaxConsumerConfig(limit uint64) *configs.SolverConfig {
