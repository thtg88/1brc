...
func TestAvgConsumer_Start(t *testing.T) {
	t.Parallel()
	...
	for _, testCase := range tests {
		...
		t.Run(tc.description, func(t *testing.T) {
			...
			config := buildAvgConsumerConfig(tc.expectedRecordsConsumed)

			consumer := avgconsumer.NewAvgTempConsumer(dataChannel, mockLogger, config)
...
func buildAvgConsumerConfig(limit uint64) *configs.SolverConfig {
