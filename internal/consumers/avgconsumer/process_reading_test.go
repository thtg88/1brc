...
func TestAvgConsumer_ProcessReading(t *testing.T) {
	t.Parallel()
	...
	t.Run("CalculateAverageForEachReading false", func(t *testing.T) {
		...
		t.Run("successful reading processing", func(t *testing.T) {
			...
			consumer := buildAvgConsumer(&configs.SolverConfig{Debug: true}, logger)
	...
	t.Run("CalculateAverageForEachReading true", func(t *testing.T) {
		...
		t.Run("successful reading processing", func(t *testing.T) {
			...
			logger := loggermock.NewLoggerMock()
			consumer := buildAvgConsumer(config, logger)
			reading := builders.NewTemperatureReadingBuilder().WithTestValues().Build()
	...
	t.Run("reading processing with debug off does not log", func(t *testing.T) {
		...
		logger := loggermock.NewLoggerMock()
		consumer := buildAvgConsumer(&configs.SolverConfig{}, logger)
		reading := builders.NewTemperatureReadingBuilder().WithTestValues().Build()
